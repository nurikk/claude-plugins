  ...
  tools: [
    {
      name: 'reply',
      description:
        'Reply on Telegram. Pass chat_id from the inbound message. For forum topics, also pass message_thread_id. Optionally pass reply_to (message_id) for threading, and files (absolute paths) to attach images or documents.',
      inputSchema: {
        type: 'object',
        properties: {
          chat_id: { type: 'string' },
  ...
  try {
    switch (req.params.name) {
      case 'reply': {
        ...
        const thread_id = args.message_thread_id != null ? Number(args.message_thread_id) : undefined
...
async function handleInbound(
  ...
  // Forum topic support — message_thread_id identifies which topic the message belongs to.
